...
class selectRoomWNTForm(forms.Form):
    room = forms.ModelChoiceField(queryset=Room.objects.filter(nextTenant=None), required=True, label="Chambre", widget=autocomplete.ModelSelect2(url='gestion:noNextTenantRoomAutocomplete'))
